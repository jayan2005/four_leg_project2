	...
	public synchronized Command processCommand(LockRequestCommand command, Connection aConnection) {
		...
		if (userRegistration != null && userRegistration.getUserName().equals(command.getUsername())) {
			if (userRegistration.getRequestTime() < command.getRequestTime()) {
				...
				// request
				return new LockDeniedCommand(command.getUsername(), command.getSecret());
			} else {
				// Fail the Registration in progress
				LockResponseListener lockResponseListener = Control.getInstance()
						.getLockResponseListener(command.getUsername());
		...
		// Local check is complete. Now broadcast and wait for responses
		...
		if (noOfServers == 0) {
			...
			lockAllowed = broadcastAndProcessLockRequest(command, false, aConnection);
		}
		if (lockAllowed) {
